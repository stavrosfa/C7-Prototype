...
public partial class TechBox : TextureButton {
	...
	public override void _Ready() {
		...
		ImageTexture techIcon = TextureLoader.Load("tech_icons.small", tech, useCache: true);
		TextureRect textureRect = new() {
			Texture = techIcon
		};
		textureRect.SetPosition(new Vector2(12, 32));
		...
		int offsetX = techIcon.GetWidth() + 7;
		int offsetY = 0;
		int counter = 0;

		// TODO : cache these textures. A simple static dict won't work
		List<ImageTexture> techEffects = TechEffectTextures();

		for (int i = 0; i < techEffects.Count; i++) {
			TextureRect tr = new() {
				Texture = techEffects.ElementAt(i),
			};
	...
	private void ShowTooltip() {
		...
		if (TruncatedToFullTechTextMap.TryGetValue(tech.Name, out string ft))
			this.TooltipText = $"{ft}";
	}
	...
	private string TruncateAndCacheName(string input, int limit) {
		...
		if (input.Length > limit) {
			...
			sb.Append(input.Trim().Substring(0, limit - 1));
			sb.Append("...");
			return sb.ToString();
		}
	...
	private string FormatStringForLuaEntry(string input) {
		input = input.Replace("'", "");
		string output = Regex.Replace(input, @"[^\w\d]", "_").ToLower();
		return output;
	}
}
